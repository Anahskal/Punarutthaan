...
import time
import sys    #gets informationa about system and python version
depth = 0
...
class game:
    def __init__(self, board_size, pit_count):   #stores info about board and score of player
        self.game_board = boardTemp.board(board_size, pit_count)
        Screen.player_score = [0, 0]
        self.current_player = 0


    def switchPlayer(self): #switiching from player to bot and vice versa
        '''
      moves to the next player
      '''
        self.current_player = (self.current_player + 1) % 2   #Since you typically want the player index to cycle between 0 and 1 (for a two-player game), it takes the remainder when divided by 2, achieved by using the modulus operator % 2.
       #For instance, if self.current_player is 0, it becomes 1. If it's 1, it becomes 0, effectively switching between players.
    ...
    def Naiveplay2(self):
        #this method implements a simple strategy where the player selects a random pit from the available options and makes a move. It's labeled as "Naive" because it doesn't employ any sophisticated strategy beyond random selection.
    ...
    def drawText(self, text, color, rect, font, aa=False, bkg=None):  #beda
        print(rect)
    ...
    def __main__(self):
        ...
        # while isGame:
        while isGame & self.isGame():         #whether the game loop should continue
            for event in pygame.event.get():    #iterates over all the events that have occurred since the last iteration
                if event.type == pygame.QUIT:  #checks if the current event is a quit event
                    pygame.quit()
                    sys.exit()
                    isGame = False
                    ...
                    for event in pygame.event.get():
                        ...
                        if event.type == pygame.MOUSEBUTTONDOWN:
                            ...
                            if pos[0] > 97 and pos[0] < 186 and pos[1] > 373 and pos[1] < 394:
                                ...
                                text = font2.render("X",1, (100,40,0))
                            ...
                            if pos[0] > 297 and pos[0] < 386 and pos[1] > 372 and pos[1] < 400:
                                ...
                                pygame.display.update()
                                flag2 = 1
                                while flag2:     #acts according to input
                                    if event.type == pygame.QUIT:
                                        pygame.quit()
                                        sys.exit()
                                        isGame = False
            ...
            pygame.draw.rect(Screen.screen, (77, 26, 0), [30, 260, 50, 50], border_radius=10)
            pygame.draw.rect(Screen.screen, (77, 26, 0), [110, 260, 50, 50], border_radius=10)
            pygame.draw.rect(Screen.screen, (77, 26, 0), [190, 260, 50 ,50], border_radius=10)
            pygame.draw.rect(Screen.screen, (77, 26, 0), [270, 260, 50, 50], border_radius=10)
            pygame.draw.rect(Screen.screen, (77, 26, 0), [350, 260, 50, 50], border_radius=10)
            pygame.draw.rect(Screen.screen, (77, 26, 0), [430, 260, 50, 50], border_radius=10)
            pygame.draw.rect(Screen.screen, (255, 85, 0), [30, 360, 50, 50], border_radius=10)
            pygame.draw.rect(Screen.screen, (255, 85, 0), [110, 360, 50, 50],border_radius=10)
            pygame.draw.rect(Screen.screen, (255, 85, 0), [190, 360, 50, 50], border_radius=10)
            pygame.draw.rect(Screen.screen, (255, 85, 0), [270, 360, 50, 50], border_radius=10)
            pygame.draw.rect(Screen.screen, (255, 85, 0), [350, 360, 50,50], border_radius=10)
            pygame.draw.rect(Screen.screen, (255, 85, 0), [430, 360, 50, 50], border_radius=10)
            ...
            text = font.render(
                "You: " + str(Screen.player_score[0]), 1, (102, 0, 0))
            Screen.screen.blit(text, (70, 450))

            text = font.render("Computer: " + str(Screen.player_score[1]), 1,(102, 0, 0))
            Screen.screen.blit(text, (230, 450))

            pygame.display.update()
            if self.current_player == 1:
                temp = "Computer move, proceed"
                text = font.render(temp, 1, (102, 0, 0))
                Screen.screen.blit(text, (50, 180))

            else:
                temp = "Your move, proceed"
                text = font.render(temp, 1, (102, 0, 0))
                Screen.screen.blit(text, (100, 180))

            pygame.display.update()

#waits for a mouse click event to occur then responds accordingly
            mouseClick = False
            ...
            for event in pygame.event.get():        #not needed
                if event.type == pygame.QUIT:
                    pygame.quit()
                    sys.exit()
                    isGame = False

        if self.getCurrentWinner() == 0:        #optional
            Screen.screen.fill((0,0,0))
            pygame.draw.rect(Screen.screen, (93, 121, 125), [50, 200, 400, 200], 5, 5)
            text = font2.render("YOU WIN!!!",1,(102, 0, 0))
            Screen.screen.blit(text, (120,275))
        else:
            Screen.screen.fill((0, 0, 0))
            pygame.draw.rect(Screen.screen, (93, 121, 125), [50, 200, 400, 200], 5, 5)
            text = font2.render("GAME OVER!", 1, (102, 0, 0))
