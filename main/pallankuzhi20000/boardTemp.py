...
class board:
    ...
    def move(self, start):
        ...
        for coin in range(coins):   #needed
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    if event.type == pygame.QUIT:
                        pygame.quit()
        ...
        if self.board_list[next_pit] == 0:     #needed

            text = font.render("player takes the next pit",1,(102, 0, 0))
            Screen.screen.blit(text, (75,200))
            score_pit = self.nextPit(next_pit)
            text = font.render(str(self.board_list[score_pit]),1,(255,255,255))
            size = self.board_size
